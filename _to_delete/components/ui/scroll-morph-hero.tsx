...
// --- FlipCard Component ---
const IMG_WIDTH = 72;  // Square aspect suits logos better than portrait
const IMG_HEIGHT = 72;
    ...
    return (
        ...
        >
            ...
            >
                {/* Front Face */}
                <div
                    className="absolute inset-0 h-full w-full overflow-hidden rounded-xl shadow-lg bg-white flex items-center justify-center p-1.5"
                ...
                >
                    <div className="text-center">
                        <p className="text-[8px] font-bold text-blue-400 uppercase tracking-widest mb-1">{name}</p>
                    </div>
                </div>
            </motion.div>
        </motion.div>
...
// Tool / collaboration brand logos (served from /assets/Logos)
const IMAGES = [
    "/assets/Logos/slack.png",
    "/assets/Logos/win-office.png",
    "/assets/Logos/google-workspace.png",
    "/assets/Logos/telegram.png",
    "/assets/Logos/asana.png",
    "/assets/Logos/monday.png",
    ...
    "/assets/Logos/quickbooks.png",
    "/assets/Logos/basecamp.png",
    "/assets/Logos/loom.png",
    "/assets/Logos/figma.png",
    "/assets/Logos/whatsapp.png",
    "/assets/Logos/meta-business.png",
    "/assets/Logos/gohighlevel.png",
];

// Display names aligned to the IMAGES array (shown on card hover/flip)
const NAMES = [
    "Slack",
    "Microsoft Office",
    "Google Workspace",
...
export default function IntroAnimation() {
    ...
    useEffect(() => {
        ...
        const handleWheel = (e: WheelEvent) => {
            // Once the full intro animation has completed, release wheel capture
            // so the user can scroll natively into the rest of the site.
            if (releasedRef.current) return;

            // Prevent default to stop browser overscroll/bounce while the
            // virtual-scroll card animation is still in progress.
            e.preventDefault();
            ...
            // Release once the animation has reached its end.
            if (newScroll >= MAX_SCROLL) {
                releasedRef.current = true;
                setReleased(true);
            } else if (newScroll >= MAX_SCROLL * 0.85) {
                setNearEnd(true);
            }
        };
    ...
    // --- Content Opacity ---
    // Delay the title/subtitle until after the arc has fully formed (scroll 600)
    // and the user has scrolled further through the shuffle phase. The wide
    // range + dedicated spring gives a long, smooth fade-in.
    const rawContentOpacity = useTransform(virtualScroll, [800, 1800], [0, 1]);
    const rawContentY = useTransform(virtualScroll, [800, 1800], [40, 0]);
    const contentOpacity = useSpring(rawContentOpacity, { stiffness: 15, damping: 25 });
    const contentY = useSpring(rawContentY, { stiffness: 15, damping: 25 });

    return (
        <div ref={containerRef} data-section="Hero · IntroAnimation" data-file="components/ui/scroll-morph-hero.tsx" className="relative w-full h-full bg-transparent overflow-hidden">
            {/* Container */}
            <div className="flex h-full w-full flex-col items-center justify-center perspective-1000">
                ...
                <div className="absolute z-0 flex flex-col items-center justify-center text-center pointer-events-none top-1/2 -translate-y-1/2">
                    ...
                    >
                     <br/> The Ultimate Platform of Virtual Employees. <br className="hidden md:block" /> 
                    </motion.p>
                </div>
                ...
                >
                    <h2 className="text-3xl md:text-5xl font-semibold text-gray-900 tracking-tight mb-4" style={{ fontFamily: "var(--font-lora)", fontStyle: "italic" }}>
                ...
                <div className="relative flex items-center justify-center w-full h-full">
                    {/* Continue hint — appears once the card animation nears completion */}
                    <motion.div
                        initial={{ opacity: 0, y: 8 }}
                        animate={nearEnd && !released ? { opacity: 0.9, y: 0 } : { opacity: 0, y: 8 }}
                        transition={{ duration: 0.4 }}
                        className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 pointer-events-none flex flex-col items-center gap-2 text-gray-700"
                    >
                        <span className="text-[10px] font-bold tracking-[0.25em] uppercase">Continue</span>
                    ...
                    {IMAGES.slice(0, TOTAL_IMAGES).map((src, i) => {
                        ...
                        if (introPhase === "scatter") {
                            ...
                            const lineSpacing = 80; // 72px card width + 8px gap
